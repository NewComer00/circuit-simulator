...
electrons_pos(:,2) = randi([1,WIRE_WIDTH], ELECTRONS_NUM, 2)
...
while true
	...
	elecs_pos_idx = sub2ind( ...
		size(RESISTANCE_INFO),electrons_pos(:,2),electrons_pos(:,1));
	res_elecs_on = RESISTANCE_INFO(elecs_pos_idx);
	electrons_vel[rand(ELECTRONS_NUM,1)<=res_elecs_on,:] = [0,0]
